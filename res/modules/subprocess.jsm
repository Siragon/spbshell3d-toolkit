...
//     );
    ...
    */
    ...
    function createStdinWriter() {
        ...
        stdinWorker = new ChromeWorker("resource://siragon/modules/subprocess_worker_win.js");
    ...
    function createReader(pipe, name, callbackFunc) {
        var worker = new ChromeWorker("resource://siragon/modules/subprocess_worker_win.js");
...
function subprocess_unix(options) {
    ...
    function createStdinWriter() {
        debugLog("Creating new stdin worker\n");
        stdinWorker = new ChromeWorker("resource://siragon/modules/subprocess_worker_unix.js");
        stdinWorker.onmessage = function(event) {
            switch (event.data.msg) {
            case "info":
     ...
     */
    function createReader(pipe, name, callbackFunc) {
        var worker = new ChromeWorker("resource://siragon/modules/subprocess_worker_unix.js");
        worker.onmessage = function(event) {
            switch(event.data.msg) {
            case "data":
                debugLog("got "+event.data.count+" bytes from "+name+"\n");
